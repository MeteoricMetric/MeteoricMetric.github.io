...
- ✓ **Manual theme-toggle UI button** + localStorage persistence — `ThemeToggle.astro` in the footer cycles system → light → dark → system. No-FOUC inline script in BaseHead applies the saved theme before first paint. Survives Astro view transitions via `astro:after-swap`.
